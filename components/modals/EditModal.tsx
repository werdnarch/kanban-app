...
import { editBoardById } from "@/app/boards/[id]/actions";
...
export default function EditModal({ active, setActive, board }: PopupProps) {
  const {
    register,
    handleSubmit,
    trigger,
    ...
    if (board) {
      const columns =
        board.columns?.length > 0
          ...
          : [
              { id: crypto.randomUUID(), column_name: "Todo" },
              { id: crypto.randomUUID(), column_name: "Doing" },
              { id: crypto.randomUUID(), column_name: "Done" },
            ];
  ...
  const editBoardMutation = useMutation({
    mutationFn: editBoardById,
    onSuccess: (data) => {
      toast.success(data.message);
      const newBoard = data.data.updatedBoard;
  ...
  const { isPending } = editBoardMutation;
  ...
  const onSubmit: SubmitHandler<FormType> = (data) => {
    ...
    editBoardMutation.mutate({ id: board.id, board: editedboard });
  };
    ...
    <div className="flex flex-col gap-4 p-6">
      ...
      >
        ...
        <BlueButton
          ...
          type="submit"
          className={`w-full ${
            !isDirty ? "opacity-70 pointer-events-none" : "opacity-100"
          }`}
        >
          {isPending ? (
